...
def grade(x_mi: np.ndarray, y_ft: np.ndarray, window_mi: float) -> np.ndarray:
...
def main(argv: list[str] | None = None) -> int:
    ...
    grades = [grade(x, y, w) for w in args.windows]
    ...
    for lo, hi in zip(edges[:-1], edges[1:]):
        sel = (x >= lo) & (x <= hi)
        print(f"  {lo:.2f}-{hi:.2f} mi: mean grade {broad[sel].mean():5.1f}%"
              f"  max {broad[sel].max():5.1f}%")
    return 0
